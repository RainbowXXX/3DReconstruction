...
int test1()
{
  std::string sImageDir = input_dir,
    sfileDatabase = camera_file_params,
    sOutputDir = matches_dir,
    sKmatrix;
        ...
        switch (eGeometricModelToCompute) {
            ...
            case ESSENTIAL_MATRIX: {
                ...
                for (const auto &pairwisematches_it: map_GeometricMatches) {
                    const size_t putativePhotometricCount = map_PutativeMatches.find(pairwisematches_it.first)->second.
                            size();
